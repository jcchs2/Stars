...
# Check if the current script is the latest version
if ($global:CurrentScriptVersion -ne $scriptVersion) {
    # Set the global variable to indicate the script version
    $global:CurrentScriptVersion = $scriptVersion

    # Download and extract the latest version of the script from a .zip archive and run it
    try {
        ...
        for ($i = 0; $i -lt $retryCount; $i++) {
            try {
                Write-Host "Attempting to download the latest version of the script archive (attempt $($i + 1) of $retryCount)..."
                Invoke-WebRequest -Uri $latestArchiveUrl -OutFile $localArchivePath
                Write-Host "Downloaded the latest version of the script archive successfully."

                # Extract the .zip archive
                Write-Host "Extracting the script from the archive..."
                Expand-Archive -Path $localArchivePath -DestinationPath $env:TEMP -Force
                
                # Run the extracted script
                if (Test-Path $localScriptPath) {
                    Write-Host "Running the latest version of the script from $localScriptPath"
                    . $localScriptPath
                    Cleanup-TempFiles $localArchivePath $localScriptPath
                    exit
                } else {
                    Write-Error "Failed to find the extracted script at $localScriptPath"
                }
            } catch {
                Write-Error "Failed to download or extract the latest version of the script: $_"
                Start-Sleep -Seconds $retryDelay
            }
        }

        Write-Error "Failed to download and extract the latest version of the script after $retryCount attempts."
    } catch {
        Write-Error "An unexpected error occurred: $_"
    }
} else {
    Write-Host "The script is already running the latest version ($scriptVersion)."
}
...
function Show-MainMenu {
    ...
    Write-Host "3. Basic Computer Setup" -ForegroundColor Cyan
    Write-Host "4. Stars and Strikes" -ForegroundColor Cyan
...
$appConfig = @{
    ...
    "Adobe Reader" = @{
        Paths = @(
            ...
            # Add additional common Adobe Reader paths
            "${env:ProgramFiles(x86)}\Adobe\Reader\AcroRd32.exe",
            "${env:ProgramFiles}\Adobe\Reader\AcroRd32.exe"
        )
    ...
    "PowerPoint" = @{
        Paths = @(
            ...
            # Office 2013 paths
            "${env:ProgramFiles}\Microsoft Office\Office15\POWERPNT.EXE",
            "${env:ProgramFiles(x86)}\Microsoft Office\Office15\POWERPNT.EXE"
        )
    }
    "Outlook" = @{
        Paths = @(
 ...
 # Add function to detect Office installation path
        function Get-OfficeInstallPath {
            ...
            foreach ($basePath in $possiblePaths) {
                if (Test-Path $basePath) {
                    $officeFolders = Get-ChildItem $basePath -Directory | Where-Object { $_.Name -match "Office\d+" }
                    foreach ($folder in $officeFolders) {
                        Write-Host "Found Office installation in: $($folder.FullName)" -ForegroundColor Green
        ...
        # Before processing apps, detect Office path
        $officePath = Get-OfficeInstallPath
        if ($officePath) {
            Write-Host "Detected Office installation at: $officePath" -ForegroundColor Green
            # Update paths in appConfig if needed
            $officeApps = @("Word", "Excel", "PowerPoint", "Outlook")
            foreach ($app in $officeApps) {
                $executableName = switch ($app) {
                    "Word" { "WINWORD.EXE" }
                    "Excel" { "EXCEL.EXE" }
                    "PowerPoint" { "POWERPNT.EXE" }
                    "Outlook" { "OUTLOOK.EXE" }
                }
                $fullPath = Join-Path $officePath $executableName
                if (Test-Path $fullPath) {
                    $appConfig[$app].Paths = @($fullPath) + $appConfig[$app].Paths
                }
            }
        }
...
foreach ($appName in @("Chrome", "Edge", "Adobe Reader", "Word", "Excel", "PowerPoint", "Outlook", "Teams")) {
    ...
    try {
        ...
        elseif ($null -ne $appInfo.PSObject.Properties['InstallFunction'] -and 
                $null -ne $appInfo.InstallFunction) {
            Write-Host "$appName is not installed. Installing now..." -ForegroundColor Yellow 
            try {
                ...
                if ($installSuccess) {
                    ...
                    Write-Host "Installation of $appName failed: Installation function returned false" -ForegroundColor Red
                }
            }
            catch {
                Write-Host "Installation of $appName failed with error: $($_.Exception.Message)" -ForegroundColor Red
            }
        }
        else {
            Write-Host "Could not find $appName installation. App will be skipped." -ForegroundColor Yellow
        }
    }
    catch {
